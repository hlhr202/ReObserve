...
// action mapper map action (stream) to next state
const actionMapper: IActionMapper<ICounter> = (action$, context$: typeof counter$) => {
	return merge<ICounter>(
		action$.pipe(
			filter((action: ICounterAction) => action.type === 'INCREMENT'),
			map((action: ICounterAction) => ({
				counter: context$.current.counter + (action.payload || 0),
			}))
		),
		action$.pipe(
